//! # cargo-dkdol — GameCube / Wii build tool
//!
...
//! cargo dkdol run   [--release] [-p <pkg>] [--example <name>] [--dolphin <path>] [--wii]
...
fn main() {
    ...
    let result = match subcmd {
        ...
        other => {
            eprintln!("cargo-dkdol: unknown subcommand '{}'", other);
            eprintln!("Run 'cargo dkdol help' for usage.");
            Err(1i32)
        }
    };
...
fn cmd_build(args: &[String]) -> Result<(), i32> {
    ...
    if !st.success() { eprintln!("cargo-dkdol: build failed"); return Err(2); }
...
fn locate_elf(root: &Path, tgt: &str, prof: &str, b: &BuildArgs) -> Result<PathBuf, i32> {
    ...
    newest_elf(&base).ok_or_else(|| {
        eprintln!("cargo-dkdol: can't find ELF in {}", base.display());
...
fn run_elf2dol(root: &Path, elf: &Path, dol: &Path) -> Result<(), i32> {
    let st = Command::new("cargo")
        ...
        .arg(elf).arg(dol)
        .status()
        .map_err(|e| { eprintln!("cargo-dkdol: elf2dol failed: {}", e); 3i32 })?;
    if !st.success() { eprintln!("cargo-dkdol: elf2dol failed"); return Err(3); }
    Ok(())
}
...
fn cmd_dol(args: &[String]) -> Result<(), i32> {
    let elf = args.first().ok_or_else(|| {
        eprintln!("Usage: cargo dkdol dol <elf> [output.dol]");
        1i32
    })?;
    let elf = PathBuf::from(elf);
    if !elf.exists() { eprintln!("cargo-dkdol: not found: {}", elf.display()); return Err(4); }
...
fn cmd_run(args: &[String]) -> Result<(), i32> {
    ...
    if !dol.exists() { eprintln!("cargo-dkdol: DOL not found: {}", dol.display()); return Err(4); }
    ...
    Command::new(&dolphin)
        ...
        .status()
        .map_err(|e| {
            eprintln!("cargo-dkdol: cannot launch '{}': {}", dolphin, e);
...
fn cmd_new(args: &[String]) -> Result<(), i32> {
    let name = args.first().ok_or_else(|| {
        eprintln!("Usage: cargo dkdol new <project-name>");
        1i32
    })?;
    let dir = PathBuf::from(name);
    if dir.exists() { eprintln!("cargo-dkdol: '{}' already exists", name); return Err(5); }
    scaffold(&dir, name)?;
    println!();
    say("Created", &format!("GameCube project '{}'", name));
    println!("  cd {name}");
    println!("  cargo dkdol build --release --example hello");
    println!("  cargo dkdol run   --example hello");
    println!();
    Ok(())
}
...
A GameCube homebrew project built with [devkit-dol](https://github.com/your-org/devkit-dol).

## Build

```sh
cargo dkdol build --release --example hello
cargo dkdol run   --example hello
```

Requires `cargo-dkdol` to be installed: `cargo install --path vendor/devkit-dol/tools/cargo-dkdol`
"#))?;
...
const HELP_MAIN: &str = "\
cargo-dkdol — GameCube/Wii build tool  (devkit-dol)

USAGE:
    cargo dkdol <SUBCOMMAND> [OPTIONS]
...
EXAMPLES:
    cargo dkdol build --release --example spinning_triangle
    cargo dkdol run   --example hello_world
    cargo dkdol dol   target/powerpc-gekko-eabi/release/my_game
    cargo dkdol new   my_game
";

const HELP_BUILD: &str = "\
cargo dkdol build — Cross-compile and convert ELF → DOL

USAGE:
    cargo dkdol build [OPTIONS] [-- CARGO_ARGS...]
...
const HELP_DOL: &str = "\
cargo dkdol dol — Convert an ELF binary to GameCube DOL format
...
const HELP_RUN: &str = "\
cargo dkdol run — Build, convert, then launch in Dolphin Emulator

USAGE:
    cargo dkdol run [BUILD_OPTIONS] [--dolphin <path>]
...
const HELP_NEW: &str = "\
cargo dkdol new — Scaffold a new GameCube homebrew project
...
// ── Embedded target specs and linker script (used by `cargo dkdol new`) ─────────
