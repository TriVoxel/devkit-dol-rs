//! # gc-crash — GameCube/Wii crash handler
//!
...
//! ## Usage
//!
//! Add `gc-crash` to your `[dependencies]` and call [`init`] once during
//! startup, after `gc_rt::exception::init()`:
//!
//! ```rust,no_run
//! unsafe {
//!     gc_rt::exception::init();
//!     gc_crash::init();  // registers crash handler for all fatal exceptions
//! }
//! ```
//!
//! ## Stack trace
...
use gc_rt::exception::{Exception, ExcCtx};
use core::fmt::Write;
...
///
/// Call this once during startup after [`gc_rt::exception::init`].
/// When a fatal exception fires with no user handler, instead of halting,
/// this displays a crash screen.
///
/// # Safety
/// Must be called after `gc_rt::exception::init()`.
pub unsafe fn init() {
    use gc_rt::exception::{Exception::*, register};
...
unsafe fn display_loop(fb_ptr: *mut u32, buf: &LineBuffer) -> ! {
    ...
    loop {
        ...
        gc_rt::timer::delay_ms(80); // ~12 fps for the crash display
