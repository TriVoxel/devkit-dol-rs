//! # gc-vfs — GameCube/Wii Virtual Filesystem
//!
//! `gc-vfs` is a **mandatory** part of the devkit. It always provides:
//!
...
//! features are enabled (or `all-fs` for all of them), `gc-fs` is pulled in
...
//! # Cargo.toml
//! [dependencies]
//! gc-vfs = { path = "…", features = ["all-fs"] }
//! ```
//!
//! ```rust,no_run
//! use gc_vfs::{self as vfs, O_RDONLY, O_RDWR, O_CREAT, MountOptions};
//!
//! unsafe {
...
pub use gc_hal::si::{Port, PadState, PadResult, Buttons};

#[cfg(feature = "_fs")]
pub use gc_fs::{FsKind, Metadata};
...
pub enum Error {
    ...
    /// (add `gc-vfs = { features = ["all-fs"] }` to `Cargo.toml`).
    FsNotAvailable,
}
...
#[cfg(feature = "_fs")]
impl From<gc_fs::FsError> for Error {
    fn from(e: gc_fs::FsError) -> Self {
        use gc_fs::FsError::*;
        match e {
            Io            => Error::Io,
...
///
/// 16 bytes, `repr(C)` — safe to read into with [`read`] via
/// `bytemuck::bytes_of_mut(&mut state)`.
#[repr(C)]
#[derive(Clone, Copy, Debug, Default)]
pub struct ControllerState {
    /// Pressed buttons (same bits as [`gc_hal::si::Buttons`]).
    pub buttons:   u16,
    ...
    pub trigger_l: u8,
    pub trigger_r: u8,
    /// 1 = controller present, 0 = nothing plugged in.
    pub connected: u8,
    _pad: [u8; 7],
}
...
enum VolumeInner {
    ...
    Empty,
    #[cfg(feature = "fat")]
    Fat(gc_fs::fat::FatVolume<gc_hal::sd::SdCard>),
    #[cfg(feature = "ext2")]
    Ext2(gc_fs::ext2::Ext2<gc_hal::sd::SdCard>),
    #[cfg(feature = "iso9660")]
    Iso(gc_fs::iso9660::Iso9660<gc_hal::dvd::DvdDisk>),
    #[cfg(feature = "dvd")]
    GcDvd(gc_fs::dvd::GcDvd<gc_hal::dvd::DvdDisk>),
    #[cfg(feature = "memcard")]
    MemCard(gc_fs::memcard::MemCardFs),
}
...
enum VfsFile {
    ...
    /// A GC controller port read as a character device.
    Controller { port: u8 },
...
/// `gc_rt` tick at last [`poll`] call per device kind.
...
fn dev_kind_for(dev_prefix: &str) -> DevKind {
    match dev_prefix {
        ...
        "/dev/hid/p1/std"| "/dev/hid/p1/kbd" => DevKind::HidPort(0),
        "/dev/hid/p2/std"| "/dev/hid/p2/kbd" => DevKind::HidPort(1),
        "/dev/hid/p3/std"| "/dev/hid/p3/kbd" => DevKind::HidPort(2),
        "/dev/hid/p4/std"| "/dev/hid/p4/kbd" => DevKind::HidPort(3),
...
fn port_for(dev_prefix: &str) -> Option<u8> {
    match dev_prefix {
        "/dev/hid/p1/std" | "/dev/hid/p1/kbd" => Some(0),
        "/dev/hid/p2/std" | "/dev/hid/p2/kbd" => Some(1),
        "/dev/hid/p3/std" | "/dev/hid/p3/kbd" => Some(2),
        "/dev/hid/p4/std" | "/dev/hid/p4/kbd" => Some(3),
        _ => None,
    }
}
    ...
    match dev_prefix {
        "/dev/sd/sp" | "/dev/sd/m1" | "/dev/sd/m2" => {
            ...
            #[cfg(feature = "fat")]
            {
                ...
                if card.init().is_ok() {
                    match gc_fs::fat::FatVolume::mount(card) {
                        Ok(fv) => {
            ...
            #[cfg(feature = "ext2")]
            {
                ...
                if card.init().is_ok() {
                    let jmode = match (opts.journal, opts.readonly) {
                        (JournalMode::Disable, _) => gc_fs::ext2::JournalMode::Ignore,
                        (JournalMode::Replay, _)  => gc_fs::ext2::JournalMode::Ignore,
                        _                          => gc_fs::ext2::JournalMode::RequireClean,
                    };
                    match gc_fs::ext2::Ext2::mount_opts(card, jmode) {
                        Ok(ev) => {
        ...
        "/dev/dvd" => {
            ...
            #[cfg(feature = "dvd")]
            {
                if let Ok(dv) = gc_fs::dvd::GcDvd::mount(DvdDisk) {
            ...
            #[cfg(feature = "iso9660")]
            {
                if let Ok(iv) = gc_fs::iso9660::Iso9660::mount(DvdDisk) {
        ...
        "/dev/mem1" | "/dev/mem2" => {
            #[cfg(feature = "memcard")]
            {
                let mc_slot = if dev_prefix == "/dev/mem1" {
                    gc_hal::memcard::CardSlot::A
                } else {
                    gc_hal::memcard::CardSlot::B
                };
                match gc_fs::memcard::MemCardFs::mount(mc_slot) {
                    Ok(mc) => {
...
unsafe fn do_read(file: &mut VfsFile, buf: &mut [u8]) -> Result<usize> {
    match file {
        ...
        VfsFile::Controller { port } => {
            ...
            let state = match gc_hal::si::read_pad(pad_port) {
                PadResult::Ok(pad) => ControllerState::from_pad(&pad),
                PadResult::NoController | PadResult::Error => ControllerState::default(),
            };
            // SAFETY: ControllerState is repr(C), all bit patterns valid
            let bytes = unsafe {
                core::slice::from_raw_parts(
                    &state as *const ControllerState as *const u8, want)
            };
...
/// Open a HID port by index (0–3) without going through path-split.
/// Returns the Fd, or None if the table is full.
pub(crate) unsafe fn vfs_open_hid(port: u8) -> Option<Fd> {
    let fd = alloc_fd()?;
...
unsafe fn probe_device(kind: DevKind) -> bool {
    ...
    match kind {
        ...
        DevKind::DvdDrive => {
            gc_hal::dvd::init();
            !gc_hal::dvd::cover_open()
        }
        DevKind::MemCardA => gc_hal::memcard::MemCard::probe(CardSlot::A).is_ok(),
        DevKind::MemCardB => gc_hal::memcard::MemCard::probe(CardSlot::B).is_ok(),
...
unsafe fn open_from_volume(vol_idx: usize, subpath: &str, flags: u32) -> Result<Fd> {
    ...
    match &mut VOLUMES[vol_idx].inner {
        ...
        VolumeInner::Fat(fv) => {
            if create {
                let (dir, name) = gc_fs::fat::split_dir_name(subpath)
...
#[cfg(feature = "_fs")]
pub unsafe fn stat(path: &str) -> Result<gc_fs::Metadata> {
    let (dev_prefix, subpath) = split_dev_path(path).ok_or(Error::NotFound)?;
...
///
/// `cb` receives one [`gc_fs::Metadata`] per entry. Return `false` from the
/// callback to stop early.
#[cfg(feature = "_fs")]
pub unsafe fn read_dir<F>(path: &str, cb: F) -> Result<()>
where F: FnMut(&gc_fs::Metadata) -> bool
{
    let (dev_prefix, subpath) = split_dev_path(path).ok_or(Error::NotFound)?;
    if !dev_is_present(dev_prefix) { return Err(Error::NoDevice); }
    let vol_idx = match find_mounted(dev_prefix) {
...
///
/// Equivalent to `gc_hal::si::read_pad` but exported from `gc-vfs` so
