...
///
/// # Safety
/// `dkdol_rt::exception::init()` must have been called first.
#[inline(always)]
...
#[inline(always)]
pub fn disable_irq() -> dkdol_rt::irq::IrqState {
    dkdol_rt::irq::disable()
}

/// Restore interrupts to a previously saved state.
#[inline(always)]
pub unsafe fn restore_irq(state: dkdol_rt::irq::IrqState) {
    dkdol_rt::irq::restore(state);
}
