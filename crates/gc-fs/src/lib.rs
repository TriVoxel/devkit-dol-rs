...
impl BlockDev for gc_hal::sd::SdCard {
    ...
    unsafe fn read_sector(&self, lba: u64, buf: &mut [u8]) -> Result<()> {
        ...
        gc_hal::sd::SdCard::read_sector(self, lba as u32, arr).map_err(|_| FsError::Io)
    }
    unsafe fn write_sector(&self, lba: u64, buf: &[u8]) -> Result<()> {
        if buf.len() < 512 { return Err(FsError::BufferTooSmall); }
        let arr: &[u8; 512] = (&buf[..512]).try_into().map_err(|_| FsError::BufferTooSmall)?;
        gc_hal::sd::SdCard::write_sector(self, lba as u32, arr).map_err(|_| FsError::Io)
    }
}
...
impl BlockDev for gc_hal::dvd::DvdDisk {
    ...
    unsafe fn read_sector(&self, lba: u64, buf: &mut [u8]) -> Result<()> {
        ...
        gc_hal::dvd::read(buf.as_mut_ptr(), 2048, lba * 2048).map_err(|_| FsError::Io)
    }
}
